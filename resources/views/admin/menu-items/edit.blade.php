...
@section('page-subtitle', 'Update "{{ $menuItem->name }}".')
    ...
    <div class="max-w-2xl">
         <form method="POST" action="{{ route('admin.menu-items.update', $menuItem) }}" enctype="multipart/form-data"
            ...
            <div class="p-6 space-y-5">
                ...
                {{-- Description --}}
                <div>
                    <label class="block text-sm font-medium text-stone-300 mb-1.5">Description</label>
                    <textarea name="description" rows="3"
                              class="w-full bg-stone-800 border border-stone-700 rounded-lg px-4 py-2.5 text-sm text-stone-100
                                     focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500/20 transition-colors resize-none">{{ old('description', $menuItem->description) }}</textarea>
                </div>
                ...
                {{-- Image --}}
                <div>
                    <label class="block text-sm font-medium text-stone-300 mb-1.5">Photo</label>
                    <div class="flex items-start gap-4">
                        <div id="image-preview"
                            ...
                            @if($menuItem->image)
                                <img src="{{ $menuItem->image_url }}" class="w-full h-full object-cover" alt="{{ $menuItem->name }}">
                            @else
                        ...
                        <div>
                            ...
                            <p class="text-xs text-stone-600 mt-1.5">Upload a new photo to replace the current one.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $menuItem->sort_order) }}" min="0"
                               class="w-full bg-stone-800 border border-stone-700 rounded-lg px-4 py-2.5 text-sm text-stone-100
                                      focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500/20 transition-colors">
                    </div>
                    <div>
        ...
        <form id="delete-form" method="POST" action="{{ route('admin.menu-items.destroy', $menuItem) }}" class="hidden"
              onsubmit="return confirm('Delete {{ $menuItem->name }} from the menu?')">
            @csrf @method('DELETE')
        </form>
    </div>
@endsection
